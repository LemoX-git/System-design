...
async def test_user_recipes_returns_only_current_user_recipes(service_client):
    author_login = unique_value('author')
    response = await register_user(service_client, login=author_login)
    assert response.status == 201
    response = await login_user(service_client, login=author_login)
    ...
    other_login = unique_value('other')
    response = await register_user(service_client, login=other_login)
    assert response.status == 201
    response = await login_user(service_client, login=other_login)
    assert response.status == 200
    other_token = response.json()['token']

    own_recipe = await create_recipe(service_client, author_token, title=unique_value('own'), description='Own recipe')
    assert own_recipe.status == 201
    other_recipe = await create_recipe(service_client, other_token, title=unique_value('other'), description='Other recipe')
    assert other_recipe.status == 201
    ...
    assert own_recipe.json()['id'] in recipe_ids
    assert other_recipe.json()['id'] not in recipe_ids


async def test_missing_recipe_ingredients_returns_not_found(service_client):
    response = await service_client.get('/api/v1/recipes/999999/ingredients')
    assert response.status == 404


async def test_user_lookup_is_cached(service_client):
    login = unique_value('chef')
    response = await register_user(service_client, login=login)
    assert response.status == 201

    first = await service_client.get('/api/v1/users/by-login', params={'login': login})
    second = await service_client.get('/api/v1/users/by-login', params={'login': login})
...
async def test_recipe_ingredients_are_cached_and_invalidated(service_client):
    login = unique_value('chef')
    response = await register_user(service_client, login=login)
    assert response.status == 201
    ...
    first = await service_client.get(f'/api/v1/recipes/{recipe_id}/ingredients')
    second = await service_client.get(f'/api/v1/recipes/{recipe_id}/ingredients')
    assert first.status == 200
    assert second.status == 200
    assert first.headers.get('X-Cache') == 'MISS'
    assert second.headers.get('X-Cache') == 'HIT'
    ...
    third = await service_client.get(f'/api/v1/recipes/{recipe_id}/ingredients')
    assert third.status == 200
    assert third.headers.get('X-Cache') == 'MISS'
    assert len(third.json()) == 2


async def test_login_rate_limit_returns_429(service_client):
    login = unique_value('chef')
    response = await register_user(service_client, login=login)
    assert response.status == 201
    ...
    limited = await login_user(service_client, login=login, password='wrong-password')
