...
void BeginTransaction(PGconn* connection) {
    auto result = ExecCommand(connection, "begin transaction", "BEGIN");
    (void)result;
}

void CommitTransaction(PGconn* connection) {
    auto result = ExecCommand(connection, "commit transaction", "COMMIT");
    (void)result;
}

void RollbackTransactionNoThrow(PGconn* connection) noexcept {
    if (!connection) {
        return;
    }
...
userver::formats::json::Value BuildUserRegisteredPayload(const User& user) {
    userver::formats::json::ValueBuilder payload(userver::formats::common::Type::kObject);
...
userver::formats::json::Value BuildRecipeCreatedPayload(const Recipe& recipe) {
    userver::formats::json::ValueBuilder payload(userver::formats::common::Type::kObject);
    payload["recipe_id"] = recipe.id;
    payload["author_id"] = recipe.author_id;
    payload["title"] = recipe.title;
    payload["description"] = recipe.description;
    return payload.ExtractValue();
}

userver::formats::json::Value BuildIngredientAddedPayload(const Ingredient& ingredient, int author_id) {
    userver::formats::json::ValueBuilder payload(userver::formats::common::Type::kObject);
    payload["ingredient_id"] = ingredient.id;
    payload["recipe_id"] = ingredient.recipe_id;
...
userver::formats::json::Value BuildFavoriteRecipeAddedPayload(int user_id, int recipe_id) {
    userver::formats::json::ValueBuilder payload(userver::formats::common::Type::kObject);
...
std::optional<User> RecipeStorage::RegisterUser(
    ...
    try {
        ...
        if (PQntuples(result.get()) == 0) {
            RollbackTransactionNoThrow(connection_);
            return std::nullopt;
        }

        auto user = ReadUser(*result, 0);
        StoreOutboxEvent(
            connection_,
            "UserRegistered",
            "user.registered",
...
std::optional<Recipe> RecipeStorage::CreateRecipe(
    ...
    try {
        ...
        auto recipe = ReadRecipe(*result, 0);
        StoreOutboxEvent(
            connection_,
            "RecipeCreated",
            "recipe.created",
            "recipe",
            recipe.id,
            BuildRecipeCreatedPayload(recipe)
        );
        CommitTransaction(connection_);
        return recipe;
    } catch (...) {
...
std::pair<RecipeStorage::AddIngredientStatus, std::optional<Ingredient>> RecipeStorage::AddIngredient(
    ...
    try {
        ...
        auto ingredient = ReadIngredient(*insert_result, 0);
        StoreOutboxEvent(
            connection_,
            "IngredientAdded",
            "ingredient.added",
            "recipe",
            recipe_id,
            BuildIngredientAddedPayload(ingredient, user_id)
        );
        CommitTransaction(connection_);
...
RecipeStorage::FavoriteStatus RecipeStorage::AddFavorite(int user_id, int recipe_id) {
    ...
    try {
        ...
        StoreOutboxEvent(
            ...
            "favorite_recipe",
            recipe_id,
            BuildFavoriteRecipeAddedPayload(user_id, recipe_id)
        );
        CommitTransaction(connection_);
